...
import br.com.eagravos.mobile.modelos.Cidades;
...
public abstract class GenericDb4oDAO<T extends IModel> implements IDAO<T> {
	...
	public static String DATA_BASE_FILE =
	"/data/data/br.gov.pe.saudecaruaru.inspetordigital/inspetor_digital_database.YAP";
	...
	protected EmbeddedConfiguration getConfiguration() {
		...
		// Estados
		config.common().objectClass(Estados.class).objectField("id")
				.indexed(true);
                //regionais
		config.common().objectClass(Regionais.class).objectField("id")
		.cascadeOnDelete(true);

		// cidade
		config.common().objectClass(Cidades.class).objectField("codigo")
				.indexed(true);
		config.common().objectClass(Cidades.class).objectField("estado")
				.indexed(true);
		config.common().objectClass(Cidades.class).maximumActivationDepth(3);

		// Unidades
		config.common().objectClass(UnidadeSaude.class).objectField("id")
				.indexed(true);

		// Departamentos
		config.common().objectClass(DepartamentosTb.class).objectField("id").indexed(true);
		

		// Setores
		config.common().objectClass(DepartamentosSetoresTb.class).objectField("id").indexed(true);


		// FichaLocalAtendimento
		config.common().objectClass(FichaAcidenteIdLocalAtendimento.class).objectField("numero_registro").indexed(true);
		

		// FichaAcidenteVitima
		config.common().objectClass(FichaAcidenteIdAcidentado.class)
				.objectField("id").indexed(true);

		// FichaDadosDoAcidente
		config.common().objectClass(FichaAcidenteDadosDoAcidente.class).objectField("ficha_acidente_id_acidentado_id")
				.indexed(true);
		// FichaEvolucao
		config.common().objectClass(FichaAcidenteEvolucao.class).objectField("ficha_acidente_id_acidentado_id")
				.indexed(true);

		//FichaGeoLocalização
		config.common().objectClass(FichaGeolocalizacaoAcidente.class).objectField("ficha_acidente_id_acidentado_id").indexed(true);
   	       
                //fichaAcidenteFatoresRelacionados
		config.common().objectClass(FichaAcidenteFatoresRelacionados.class).objectField("ficha_acidente_id_acidentado_id").indexed(true);
		
                // fichaGeolocalizacaoEnvolvidos no acidente
		config.common().objectClass(FichaGeolocalizacaoEnvolvidosNoAcidente.class).objectField("ficha_acidente_id_acidentado_id");
	...
	public List<T> findAllWithPagination(int limit, int offset) {
		...
		try {
			...
			Log.e(ApplicationEagravosMobile.TAG, e.getMessage());
		} finally {
			if (db != null) {
				db.close();
			}
	...
	public boolean save(T model, boolean checkExist) throws SaveException,
		...
		try {
			...
			success = false;
			db.rollback();
			Log.e(ApplicationEagravosMobile.TAG, ex.getMessage());
		} finally {
			if (db != null) {
				db.close();
			}
